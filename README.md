...
**Guard.** Checked before anything is deleted or written: `messages/` under `output_dir` may be deleted only if it is absent, empty, or holds the `.batch-fetch-window` marker file the tool writes into `messages/` on every run and nothing but regular files named `NNN.json` (three or more digits) or `.publish-*`. Anything else — a missing marker, an unknown name, a subdirectory, a symlink, or `messages/` being a file — makes the call fail with an error naming the offending entry, and nothing is deleted or written. The marker is the tool's own record; nothing else should read or write it.

**When the call fails.** The result is an MCP error (`isError: true`) and nothing under `output_dir` changes. Every Gmail call — the profile lookup, every window-listing page, every `messages.get` and `attachments.get`, and the cross-check listings — completes before the tool deletes or writes anything, so invalid input, a listing page that still fails after its retries (the error text names the query, the 1-based page and the final status, e.g. `window listing failed: query "after:… -in:spam -in:trash" page 2 status 503 after 5 attempts`), an authentication failure (401, or a 403 that is not a rate limit) on any call, and a guard refusal all leave the previous outputs byte-for-byte intact. A window whose listing is incomplete is never written. Only after all of that does the tool delete `manifest.json`, `window-metadata.json` and `messages/` and write the new run, publishing the two JSON files last and atomically; from that point only a local file-system error can interrupt it, in which case `messages/` holds the marker and the new message files and a rerun replaces that state.
